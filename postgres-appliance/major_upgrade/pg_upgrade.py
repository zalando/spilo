...
class _PostgresqlUpgrade(Postgresql):
        ...
        for d in self.query('SELECT datname FROM pg_catalog.pg_database WHERE datallowconn'):
            ...
            with get_connection_cursor(**conn_kwargs) as cur:
                ...
                if d[0] == 'postgres':
                    ...
                    cur.execute('DROP TABLE public.postgres_log CASCADE')
    ...
    def analyze(self):
        logger.info('Rebuilding statistics (vacuumdb --analyze-in-stages)')
        vacuumdb_args = ['-a', '-Z', '--analyze-in-stages', '-j', str(psutil.cpu_count())]
        if 'username' in self.config.superuser:
