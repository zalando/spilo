...
function test_inplace_upgrade_wrong_version() {
    docker_exec "$1" "PGVERSION=15 $UPGRADE_SCRIPT 3" 2>&1 | grep 'Upgrade is not required'
}

function test_inplace_upgrade_wrong_capacity() {
    docker_exec "$1" "PGVERSION=16 $UPGRADE_SCRIPT 4" 2>&1 | grep 'number of replicas does not match'
}
...
function test_successful_inplace_upgrade_to_19() {
    docker_exec "$1" "PGVERSION=19 $UPGRADE_SCRIPT 3"
}

function test_pg_upgrade_to_19_check_failed() {
    ! test_successful_inplace_upgrade_to_19 "$1"
}
...
function start_clone_with_walg_upgrade_to_19_container() {
    docker-compose run \
        -e SCOPE=upgrade3 \
        -e PGVERSION=19 \
        -e CLONE_SCOPE=demo \
...
function start_clone_with_walg_19_container() {
    docker-compose run \
        -e SCOPE=clone18 \
        -e PGVERSION=19 \
        -e CLONE_SCOPE=upgrade3 \
        -e CLONE_PGVERSION=19 \
        -e CLONE_METHOD=CLONE_WITH_WALG \
...
# TEST SUITE 1 - In-place major upgrade 15->16->...->19
# TEST SUITE 2 - Major upgrade 15->19 after wal-g clone (with CLONE_PGVERSION set)
# TEST SUITE 3 - PITR (clone with wal-g) with unreachable target (16+)
# TEST SUITE 4 - Major upgrade 15->16 after wal-g clone (no CLONE_PGVERSION)
# TEST SUITE 5 - Replica bootstrap with wal-g
# TEST SUITE 6 - Major upgrade 16->17 after clone with basebackup
# TEST SUITE 7 - Hourly log rotation
function test_spilo() {
    # TEST SUITE 1
    local container=$1

    run_test test_envdir_suffix "$container" 15
    ...
    upgrade3_container=$(start_clone_with_walg_upgrade_to_19_container) # SCOPE=upgrade3 PGVERSION=19 CLONE: _SCOPE=demo _PGVERSION=15 _TARGET_TIME=<next_min>
    log_info "[TS2] Started $upgrade3_container for testing major upgrade 15->19 after clone with wal-g"


    # TEST SUITE 4
    local upgrade_container
    upgrade_container=$(start_clone_with_walg_upgrade_container) # SCOPE=upgrade PGVERSION=16 CLONE: _SCOPE=demo _TARGET_TIME=<next_min>
    log_info "[TS4] Started $upgrade_container for testing major upgrade 15->16 after clone with wal-g"
    ...
    create_timescaledb "$container" # we don't install it at the beginning, as we do 15->19 in a clone
    ...
    # TEST SUITE 2
    log_info "[TS2] Testing in-place major upgrade 15->19 after wal-g clone"
    run_test verify_clone_upgrade "$upgrade3_container" "wal-g" 15 19
    ...
    # TEST SUITE 3
    local clone19_container
    clone19_container=$(start_clone_with_walg_19_container) # SCOPE=clone19 CLONE: _SCOPE=upgrade3 _PGVERSION=19 _TARGET_TIME=<next_hour>
    log_info "[TS3] Started $clone19_container for testing point-in-time recovery (clone with wal-g) with unreachable target on 16+"
    ...
    # TEST SUITE 3
    find_leader "$clone19_container"
    run_test verify_archive_mode_is_on "$clone19_container"


    # TEST SUITE 1
    wait_backup "$container"

    log_info "[TS1] Testing in-place major upgrade to 17->18"
    ...
    # TEST SUITE 4
    log_info "[TS4] Testing in-place major upgrade 15->16 after clone with wal-g"
    run_test verify_clone_upgrade "$upgrade_container" "wal-g" 15 16
    ...
    basebackup_container=$(start_clone_with_basebackup_upgrade_container "$upgrade_container")  # SCOPE=upgrade2 PGVERSION=17 CLONE: _SCOPE=upgrade
    log_info "[TS6] Started $basebackup_container for testing major upgrade 16->17 after clone with basebackup"
    ...
    log_info "[TS1] Testing in-place major upgrade 18->19"
    run_test test_successful_inplace_upgrade_to_19 "$container"
    wait_all_streaming "$container"
    run_test test_envdir_updated_to_x 19
    ...
    # TEST SUITE 6
    log_info "[TS6] Testing in-place major upgrade 16->17 after clone with basebackup"
    run_test verify_clone_upgrade "$basebackup_container" "basebackup" 16 17
